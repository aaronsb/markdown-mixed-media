...
- On the "validate coverage first" gate: the earlier Remaining-phases bullet gated removal on validating `beautiful-mermaid` coverage across real-world diagrams. That gate concerned the risk of *losing* the `mmdc` fallback for diagram types `beautiful-mermaid` can't yet render. This change does not lose that fallback — it only stops *bundling* it; `mmdc` remains reachable via system `PATH`. So un-bundling does not depend on the coverage validation, and the coverage risk is unchanged from before this phase. Full coverage validation remains worthwhile before any future change that would drop the `mmdc` path entirely.
